...
import { Product, Supplier, InvoiceStatus } from '../../types';
...
export const PurchaseForm: React.FC<PurchaseFormProps> = ({ onSuccess }) => {
  ...
  const onSubmit = async (data: PurchaseFormData) => {
    try {
      ...
      if (data.contact_type === 'new') {
        ...
        purchaseData.supplier_id = newSupplier.id;
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">المورّد</label>
        <select
      ...
      {contactType === 'existing' && (
        <div>
          ...
          <ul className="mt-2 max-h-40 overflow-y-auto border rounded-md bg-white">
            ...
            {filteredSuppliers.length === 0 && contactSearchTerm && (
      ...
      {contactType === 'new' && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">اسم المورّد الجديد</label>
          <input
